drop function input_live_score_game(passing_data jsonb);


create or replace function input_live_score_game(passing_data jsonb)
returns void
as
$$
declare
    ...
    table_name TEXT := '"' || matchId || '__GameMatchBByDetails' || '"';
    total_balls integer;
    live_score integer;
    totalOver INTEGER;
    balByOver INTEGER;
...
-- noBallRun ,noBallByRun, wideBallRun, wideBallByRun
    ...
    IF (passing_data ? 'runTaken') THEN
        ...
        query := format('INSERT INTO %s (
                ...
                table_name,matchId,balByOver,totalOver,live_score,scorrerId,bowling,facing,runner,extrarunner,runTaken,whereRunTaken,isSideChange);
        EXECUTE query;
    ...
    ELSIF (passing_data ? 'runNoBall') THEN
        ...
        query := format('INSERT INTO %s (
	            ...
	            VALUES (''%s'', ''%s'', ''%s'', ''%s'', ''%s'', ''%s'', ''%s'', ''%s'', ''%s'', ''%s'', ''%s'',''%s'')',
                table_name,matchId,balByOver,totalOver,live_score,scorrerId,bowling,facing,runner,isSideChange,extrarunner,runNoBall,byRunsOnNoBall);
        EXECUTE query;
...
--     1st time wide ball
    ELSIF (passing_data ? 'runWideBall') THEN
        ...
        query := format('INSERT INTO %s (
                ...
                table_name,matchId,balByOver,totalOver,live_score,scorrerId,bowling,facing,runner,isSideChange,extrarunner,runWideBall,byRunsOnWideBall,whereRunTaken);
        EXECUTE query;
    ...
    ELSIF (passing_data ? 'runsOnLegBy') THEN
        ...
        query := format('INSERT INTO %s (
	            ...
	            VALUES (''%s'', ''%s'', ''%s'', ''%s'', ''%s'', ''%s'', ''%s'', ''%s'', ''%s'', ''%s'', ''%s'',''%s'')',
                table_name,matchId,balByOver,totalOver,live_score,scorrerId,bowling,facing,runner,extrarunner,runsOnLegBy,whereRunTaken,isSideChange);
        EXECUTE query;
    ...
    ELSIF (passing_data ? 'wicketsTaken') THEN
        ...
        query := format('INSERT INTO %s (
                ...
                table_name,matchId,balByOver,totalOver,live_score,scorrerId,bowling,facing,runner,extrarunner,isSideChange,wicketsTaken,WicketType,wicketWhereTaken,wicketTakingAssistant);
        EXECUTE query;
